    ...
    caches.keys()
      .then(function (keys) {
        return Promise.all(keys.map(function (k) {
          // Only tidy up this app's own older caches. The sibling apps — and the
          // old /deep/apps/ installs, which use the bare 'timesheet-' prefix — share
          // an origin, so a blanket delete would wipe their offline copies.
          if (k === CACHE) return null;
          return k.indexOf('pwa-timesheet-') === 0 ? caches.delete(k) : null;
        }));
      })
